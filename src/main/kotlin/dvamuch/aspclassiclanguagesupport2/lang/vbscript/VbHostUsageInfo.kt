...
/**
 * Usage views outlive the read action that found their references. Keeping a smart pointer to an
 * injected PSI leaf makes later sorting and rendering restore that pointer on EDT, which requires
 * enumerating injections under read access. A host-file range is stable and navigates to the same
 * source text without retaining injected PSI.
 */
